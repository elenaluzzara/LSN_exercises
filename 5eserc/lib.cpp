...
void BlockingMethod :: Stampa(string nomefile){
...
double p_gs(double x, double y, double z){
 ...
 return pow(M_PI,-1)*exp(-2*r);
}

double p_1e(double x, double y, double z){

     double psi = (1./8.)*sqrt(2./M_PI)*exp(-sqrt(x*x + y*y + z*z)*0.5)*z;
 return pow(psi,2);
} 
