...
opportunely initialized by the initialize() function. */
	for(int i=0;i<equilibration;i++) //ciclo di riequilibrazione 
	{
		if(PIGS)   // only a PIGS polymer has a start and an end. 
		{
			brownianMotion(LEFT); //muovere polimero con moto browniano a sinistra o a destra -> la parte cinetica del propogazione è un nucleo stocastico gaussiano -> modo efficiente cancellare una delle due code e ricostruirla campionandola in modo esatto per la parte cinetica (proponiano la mossa con moto browniano) mentre metropolis accetta la parte potenziale del propagatore a seconda del peso -> questo realizza l'important sampling ossia l'efficienza massima per questa mossa 
		...
		for(int j=0;j<brownianBridgeAttempts;j++) 
			brownianBridge(); //moto browniano a estremi fissati: campionare percorso in mezzo sapendo punto di partenza e di arrivo 
	}
...
double variationalWaveFunction_second(double v)
{
	...
	double exp_meno = exp(-pow(v-mu,2)/(2*pow(sigma,2)));
	double exp_piu = exp(-pow(v+mu,2)/(2*pow(sigma,2)));
	//return 0;
	//return v*v*exp(-0.5*v*v) - exp(-0.5*v*v);
	return pow(v+mu,2)*exp_piu/pow(sigma,4) + pow(v-mu,2)*exp_meno/pow(sigma,4) - exp_meno/pow(sigma,2) - exp_piu/pow(sigma,2); 
}
...
void translation()
{
	...
	for(int i=0;i<last;i++)
	{
		...
		newcorr = potential_density_matrix(positions[i]+delta,positions[inext]+delta); //cambia
	...
	if(PIGS)
		acceptance_probability *=variationalWaveFunction(positions[0]+delta)*variationalWaveFunction(positions[timeslices-1]+delta)/(variationalWaveFunction(positions[0])*variationalWaveFunction(positions[timeslices-1])); //qui si cambia
