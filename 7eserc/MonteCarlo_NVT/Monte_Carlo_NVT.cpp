...
using namespace std;
//ho riequilibrato con 10000 step per una simu ok?
int main()
{ 
...
//stiamo campionando solo il peso di boltz configurazionale, non ci sono le velocità
...
//Read input informations
  ...
  //Tail corrections for potential energy and pressure in media -> g è approx a 1 per r > rcut
...
//measurement of g(r) -> la calcoliamo da 0 a L/2 
  igofr = 2;
  nbins = 100;
  n_props = n_props + nbins;
  bin_size = (box/2.0)/(double)nbins;
  cout << "Dimensione del bin per g(r): " << bin_size << endl;
...
double Boltzmann(double xx, double yy, double zz, int ip) //mi sto calcolando n-1 termini perchè gli altri si elidono in deltaE all'argomento dell'esponenziale del peso di Boltz
{
  double ene=0.0;
  double dx, dy, dz, dr;
...
//cycle over pairs of particles
  ...
  {
    for (int j=i+1; j<npart; ++j) //cicla una volta sola tutte le coppie
    {
...
// contribution to energy and virial
  ...
  ValoriIstantanei << walker[iv]/(double)npart + vtail << endl; 
  ValoriIstantanei.close(); 

  ValoriIstantanei.open("instant_pres.out",ios::app);
  ValoriIstantanei << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl; 
  ValoriIstantanei.close(); 
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Gofr << setw(wd) << glob_av[i]/(double)iblk; 
    }
    Gofr << endl; 
...
//g(r) per blocco finale per ogni bin
    if(iblk == nblk){ 
     for(int i = igofr; i < igofr + nbins; i++){
     r = (i-2)*bin_size;
     Gave << setw(wd) << i - 2 << setw(wd) << r+bin_size*0.5 << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gofr[i] << endl;
     }
    }
